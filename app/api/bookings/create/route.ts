...
//   3. Create bike records
//   4. Create quote record
//   5. Send quote email via Resend
//   6. If Stripe deposit: create Stripe checkout session
...
const supabase = createClient(
  ...
  process.env.SUPABASE_SERVICE_ROLE_KEY!  // service role bypasses RLS for server-side inserts
);
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ---- 4. Create quote record (if quote data available) ----
    let quoteId: string | null = null;
    if (quote) {
      // Fetch current rate card snapshot
    ...
    // ---- 6. Stripe checkout (if deposit selected) ----
  ...
  return `
    <div style="font-family:'Helvetica Neue',Arial,sans-serif;max-width:600px;margin:0 auto;background:#fff;">
      ...
      <div style="padding:32px;">
        ...
        <p style="text-align:center;font-size:13px;color:#898880;margin-top:16px;">
          A member of our team will be in touch to arrange payment and confirm your booking.
        </p>` : `
        <p style="text-align:center;font-size:13px;color:#898880;margin-top:16px;">
          Click the button above to pay your deposit and lock in your booking.
        </p>`}

        <hr style="border:none;border-top:1px solid #E8E7E5;margin:32px 0;">

        <p style="font-size:13px;color:#898880;margin:0;">
          Questions? Call or text us at <a href="tel:+61XXXXXXXXXX" style="color:#4FC1DB;">0X XXXX XXXX</a>
          or reply to this email.<br>
          <em>Note: This quote is an estimate. Final price confirmed before booking is locked in.</em>
        </p>
      </div>
    </div>
  `;
  ...
  return `
    <div style="font-family:'Helvetica Neue',Arial,sans-serif;max-width:600px;margin:0 auto;">
      <div style="background:#0D0D0D;padding:20px 24px;">
        <span style="font-size:16px;font-weight:700;color:white;text-transform:uppercase;letter-spacing:0.05em;">
          New Booking Request
        </span>
      </div>
      <div style="padding:24px;background:#F5F5F4;">
        ...
        <div style="margin-top:24px;">
          <a href="${dashboardUrl}" style="background:#4FC1DB;color:white;padding:12px 24px;border-radius:6px;font-weight:700;font-size:14px;text-decoration:none;display:inline-block;">
            View in Dashboard →
          </a>
        </div>
      </div>
